(function (root) {
  ...
  class FrameDecoder {
    constructor() { this.peakBytes = 0; this.reset(); }
    ...
    push(chunk) { return Array.from(this.frames(chunk)); }
    *frames(chunk) {
      ...
      while (offset < chunk.length) {
        ...
        if (this.kind === "headerLength") {
          ...
          this.kind = "header"; this.block = this.allocate(length, chunk.length);
          ...
          if (!length) { const frame = {header:this.header,audio:new Uint8Array()}; this.reset(); yield frame; continue; }
          if (length + chunk.length > MAX_AUDIO_BYTES) throw new Error("Encoded backlog exceeds budget");
          this.kind = "audio"; this.block = this.allocate(length, chunk.length);
        } else {
          const frame = {header:this.header,audio:this.block}; this.reset(); yield frame; continue;
        }
        this.used = 0;
      }
    }
  ...
  class StreamCursor {
    constructor(expectedCount, version = 1, sourceLengths = null) {
      ...
      this.version = version; this.sequence = 0; this.unitId = 0; this.unit = null; this.unitEnd = 0;
    ...
    accept(header, audio) {
      ...
      if (this.version === 2 && (header.protocol_version !== 2 || header.sequence !== this.sequence++)) throw new Error("Stream protocol/sequence mismatch");
      if (header.error) throw Object.assign(new Error(String(header.error)), { code: header.code });
      if (header.keepalive === true) {
        if (audio.length || Object.keys(header).some(key => !["keepalive", ...(this.version === 2 ? ["protocol_version", "sequence"] : [])].includes(key))) throw new Error("Invalid keepalive frame");
        return; // Transport liveness is neither audio nor a finalized text unit.
      }
      if (header.done === true) {
        if (audio.length || this.nextIndex !== this.expectedCount || this.unit || (this.version === 2 && header.outcome !== "completed")) throw new Error("Missing final before done");
      ...
      if (this.version === 2 && header.final !== true) {
        if (header.unit_id !== this.unitId || !Number.isInteger(header.start) || !Number.isInteger(header.end) ||
            header.start !== this.unitEnd || header.end <= header.start) throw new Error("Invalid unit source coverage");
        if (this.unit && (this.unit.start !== header.start || this.unit.end !== header.end)) throw new Error("Unit metadata changed");
        if (header.unit_final === true) {
          if (audio.length || !this.unit) throw new Error("Empty or malformed unit final");
          this.unitEnd = header.end; this.unitId++; this.unit = null; return;
        }
      ...
      if (header.final === true) {
        ...
        if (this.version === 2 && this.sourceLengths && this.unitEnd !== this.sourceLengths[this.nextIndex]) throw new Error("Incomplete source text coverage");
        this.unitEnd = 0;
